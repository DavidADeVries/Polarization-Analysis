...
%
% Output: A matrix "complete_MM" of dimensions N x M x 4 x 4, where N is
% the height of the images, M is the width of the images, and the 4 x 4 is
% your Mueller Matrix for each pixel in the image
%
...
function[complete_MM] = compute_MM_corrected_old_norm(filename)
...
complete_MM = zeros(N,M,4,4); %allocate memory for the image's MMs, each pixel have an associated 4x4 MM
...
for y=1:N
    for x=1:M
        ...
        MM = M_out / M_G; %following eqn (3) in Bueno/Campbell
        
        %MM = MM ./ MM(1,1); %normalize (can also normalize over all indices ie find max MM value in whole image, and divide all pixels' MMs by that value
        
        complete_MM(y,x,:,:) = MM; % store to pixel
    end
end

maximum = max(max(max(max(complete_MM))));

complete_MM = complete_MM ./ maximum;