...
% Choose default command line output for LocationMetadataEntry
...
if handles.eye == EyeTypes.Unknown
    handles.eye = Constants.DEFAULT_EYE_TYPE;
end
...
elseif handles.selectedQuarter == QuarterTypes.Temporal
    if handles.eye == EyeTypes.Right
        retinaShade(0,0,1,pi/4, 3*pi/4);
        retinaShade(0,0,1,-pi/4, pi/4);
        retinaShade(0,0,1,5*pi/4, 7*pi/4);
        hold on;
    elseif handles.eye == EyeTypes.Left
        retinaShade(0,0,1,pi/4, 3*pi/4);
        retinaShade(0,0,1,3*pi/4, 5*pi/4);
        retinaShade(0,0,1,5*pi/4, 7*pi/4);
        hold on;
...
elseif handles.eye == EyeTypes.Right && handles.plotFovea == false
    set(handles.MapTitle,'String','Right Eye without Fovea');
elseif handles.eye == EyeTypes.Left && handles.plotFovea == false
