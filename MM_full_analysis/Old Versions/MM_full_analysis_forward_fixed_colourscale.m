function[] = MM_full_analysis_forward_fixed_colourscale(path, file)
...
for pix_i = 1:xSize
    ...
    for pix_j = 1: ySize
        ...
        opticalRotationCol  (pix_j, 1) = real(atan((M_R(3,2)-M_R(2,3))/(M_R(2,2)+M_R(3,3)))*radToDeg);
...
write_colourbar_files(retardance, strcat(retardancePath, '_retardance'), [0,90]); %this axis bounds do not cover the mathematical range, though for our trials they cover the experimental range
write_colourbar_files(opticalRotation, strcat(retardancePath, '_optical_rotation'),[-90,90]);
write_colourbar_files(retardanceHorz, strcat(retardancePath, '_horizontal_retardance'),[-90,90]);
write_colourbar_files(retardance45deg, strcat(retardancePath, '_45deg_retardance'),[-90,90]);
write_colourbar_files(retardanceCirc, strcat(retardancePath, '_circular_retardance'),[-90,90]);
write_colourbar_files(retardanceLinear, strcat(retardancePath, '_linear_retardance'),[0,90]);
