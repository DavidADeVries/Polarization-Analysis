function[] = MM_full_analysis_forward_fixed_colourscale_testbed_reroute(path, file)
...
imageSizes = size(imread('831 Reroute/reg_831AD_set2_5location_m00.bmp'));
ySize = imageSizes(1);
xSize = imageSizes(2);

MM_pixelwise_norm = zeros(ySize,xSize,4,4);

MM_pixelwise_norm(:,:,1,1) = double(dlmread('831 Reroute/reg_831AD_set2_5location_m00.txt'));
MM_pixelwise_norm(:,:,1,2) = double(dlmread('831 Reroute/reg_831AD_set2_5location_m01.txt'));
MM_pixelwise_norm(:,:,1,3) = double(dlmread('831 Reroute/reg_831AD_set2_5location_m02.txt'));
MM_pixelwise_norm(:,:,1,4) = double(dlmread('831 Reroute/reg_831AD_set2_5location_m03.txt'));
MM_pixelwise_norm(:,:,2,1) = double(dlmread('831 Reroute/reg_831AD_set2_5location_m10.txt'));
MM_pixelwise_norm(:,:,2,2) = double(dlmread('831 Reroute/reg_831AD_set2_5location_m11.txt'));
MM_pixelwise_norm(:,:,2,3) = double(dlmread('831 Reroute/reg_831AD_set2_5location_m12.txt'));
MM_pixelwise_norm(:,:,2,4) = double(dlmread('831 Reroute/reg_831AD_set2_5location_m13.txt'));
MM_pixelwise_norm(:,:,3,1) = double(dlmread('831 Reroute/reg_831AD_set2_5location_m20.txt'));
MM_pixelwise_norm(:,:,3,2) = double(dlmread('831 Reroute/reg_831AD_set2_5location_m21.txt'));
MM_pixelwise_norm(:,:,3,3) = double(dlmread('831 Reroute/reg_831AD_set2_5location_m22.txt'));
MM_pixelwise_norm(:,:,3,4) = double(dlmread('831 Reroute/reg_831AD_set2_5location_m23.txt'));
MM_pixelwise_norm(:,:,4,1) = double(dlmread('831 Reroute/reg_831AD_set2_5location_m30.txt'));
MM_pixelwise_norm(:,:,4,2) = double(dlmread('831 Reroute/reg_831AD_set2_5location_m31.txt'));
MM_pixelwise_norm(:,:,4,3) = double(dlmread('831 Reroute/reg_831AD_set2_5location_m32.txt'));
MM_pixelwise_norm(:,:,4,4) = double(dlmread('831 Reroute/reg_831AD_set2_5location_m33.txt'));

global_max = max(max(max(max(MM_pixelwise_norm))));
m00_max = max(max(MM_pixelwise_norm(:,:,1,1)));

for pix_i = 1:xSize
    for pix_j = 1:ySize
        local_max = (max(max(MM_pixelwise_norm(pix_j,pix_i,:,:))));
        for i=1:4
            for j=1:4
                MM_pixelwise_norm(pix_j,pix_i,j,i) = MM_pixelwise_norm(pix_j,pix_i,j,i) ./ local_max;
            end
        end
    end
end
...
devriesMetric = zeros(ySize,xSize);
devriesMetricSemi = zeros(ySize,xSize);
...
for pix_i = 1:xSize
    ...
    devriesMetricCol = zeros(ySize,1);
    devriesMetricSemiCol = zeros(ySize,1);
    ...
    for pix_j = 1: ySize
        ...
        %STOKES!
%         QWP @ 141.8 w/ VLP
%         S0 = 0.5*MM(1,1) - 0.308785*MM(1,2) + 0.242990*MM(1,3) + 0.309204*MM(1,4);
%         S1 = 0.5*MM(2,1) - 0.308785*MM(2,2) + 0.242990*MM(2,3) + 0.309204*MM(2,4);
%         S2 = 0.5*MM(3,1) - 0.308785*MM(3,2) + 0.242990*MM(3,3) + 0.309204*MM(3,4);
%         S3 = 0.5*MM(4,1) - 0.308785*MM(4,2) + 0.242990*MM(4,3) + 0.309204*MM(4,4);
        S0 = 0.5*(MM(1,1)+MM(1,2)+MM(1,3)+MM(1,4));
        S1 = 0.5*(MM(2,1)+MM(2,2)+MM(2,3)+MM(2,4));
        S2 = 0.5*(MM(3,1)+MM(3,2)+MM(3,3)+MM(3,4));
        S3 = 0.5*(MM(4,1)+MM(4,2)+MM(4,3)+MM(4,4));
        ...
        devriesMetricCol(pix_j, 1) = sqrt(((MM(2,4))^2) + ((MM(4,2))^2) + ((MM(3,4))^2) + ((MM(4,3))^2));
        devriesMetricSemiCol(pix_j, 1) = sqrt(((MM(4,2))^2) + ((MM(4,3))^2));
    ...
    devriesMetric(:,pix_i) = devriesMetricCol;
    devriesMetricSemi(:,pix_i) = devriesMetricSemiCol;
end
