...
for i = 1:height(metric_labels)
    [h,p,ci,stats] = ttest2(mean_positive_values(i,:),mean_negative_values(i,:));
    mean_t_test_results(i,:) = [h,p,ci(1),ci(2),stats.tstat,stats.df,stats.sd];
    [h,p,ci,stats] = ttest2(stdev_positive_values(i,:),stdev_negative_values(i,:));
    stdev_t_test_results(i,:) = [h,p,ci(1),ci(2),stats.tstat,stats.df,stats.sd];
    [h,p,ci,stats] = ttest2(median_positive_values(i,:),median_negative_values(i,:));
    median_t_test_results(i,:) = [h,p,ci(1),ci(2),stats.tstat,stats.df,stats.sd];
    [h,p,ci,stats] = ttest2(skew_positive_values(i,:),skew_negative_values(i,:));
    skew_t_test_results(i,:) = [h,p,ci(1),ci(2),stats.tstat,stats.df,stats.sd];
end
...
% print p vals
pos_neg_label = 'Positive-Negative Un-Paired T-Test';
...
for i = 1:height(metric_labels)
    [h,p,ci,stats] = ttest2(mean_positive_values(i,:),mean_control_values(i,:));
    mean_t_test_results(i,:) = [h,p,ci(1),ci(2),stats.tstat,stats.df,stats.sd];
    [h,p,ci,stats] = ttest2(stdev_positive_values(i,:),stdev_control_values(i,:));
    stdev_t_test_results(i,:) = [h,p,ci(1),ci(2),stats.tstat,stats.df,stats.sd];
    [h,p,ci,stats] = ttest2(median_positive_values(i,:),median_control_values(i,:));
    median_t_test_results(i,:) = [h,p,ci(1),ci(2),stats.tstat,stats.df,stats.sd];
    [h,p,ci,stats] = ttest2(skew_positive_values(i,:),skew_control_values(i,:));
    skew_t_test_results(i,:) = [h,p,ci(1),ci(2),stats.tstat,stats.df,stats.sd];
end
...
% print p vals
pos_con_label = 'Positive-Control Un-Paired T-Test';
...
for i = 1:height(metric_labels)
    [h,p,ci,stats] = ttest2(mean_negative_values(i,:),mean_control_values(i,:));
    mean_t_test_results(i,:) = [h,p,ci(1),ci(2),stats.tstat,stats.df,stats.sd];
    [h,p,ci,stats] = ttest2(stdev_negative_values(i,:),stdev_control_values(i,:));
    stdev_t_test_results(i,:) = [h,p,ci(1),ci(2),stats.tstat,stats.df,stats.sd];
    [h,p,ci,stats] = ttest2(median_negative_values(i,:),median_control_values(i,:));
    median_t_test_results(i,:) = [h,p,ci(1),ci(2),stats.tstat,stats.df,stats.sd];
    [h,p,ci,stats] = ttest2(skew_negative_values(i,:),skew_control_values(i,:));
    skew_t_test_results(i,:) = [h,p,ci(1),ci(2),stats.tstat,stats.df,stats.sd];
end
...
% print p vals
neg_con_label = 'Negative-Control Un-Paired T-Test';
...
mean_signed_rank_results = zeros(height(metric_labels),4);
stdev_signed_rank_results = zeros(height(metric_labels),4);
median_signed_rank_results = zeros(height(metric_labels),4);
skew_signed_rank_results = zeros(height(metric_labels),4);

%% POS - NEG T TEST %%

for i = 1:height(metric_labels)
    [h,p,stats] = signrank(mean_positive_values(i,:),mean_negative_values(i,:));
    mean_signed_rank_results(i,:) = [h,p,stats.signrank,stats.zval];
    [h,p,stats] = signrank(stdev_positive_values(i,:),stdev_negative_values(i,:));
    stdev_signed_rank_results(i,:) = [h,p,stats.signrank,stats.zval];
    [h,p,stats] = signrank(median_positive_values(i,:),median_negative_values(i,:));
    median_signed_rank_results(i,:) = [h,p,stats.signrank,stats.zval];
    [h,p,stats] = signrank(skew_positive_values(i,:),skew_negative_values(i,:));
    skew_signed_rank_results(i,:) = [h,p,stats.signrank,stats.zval];
end
...
% print p vals
pos_neg_label = 'Positive-Negative Sign Rank Test';
...
for i=1:height(metric_labels)
   stat_test_result_print(mean_signed_rank_results(i,2), mean_vals);
   stat_test_result_print(stdev_signed_rank_results(i,2), stdev_vals);
   stat_test_result_print(median_signed_rank_results(i,2), median_vals);
   stat_test_result_print(skew_signed_rank_results(i,2), skew_vals);
end
...
for i=1:height(metric_labels)
   stat_test_result_print(mean_signed_rank_results(i,1), mean_vals);
   stat_test_result_print(stdev_signed_rank_results(i,1), stdev_vals);
   stat_test_result_print(median_signed_rank_results(i,1), median_vals);
   stat_test_result_print(skew_signed_rank_results(i,1), skew_vals);
end
...
fprintf(mean_vals, ',Sign Rank');
fprintf(stdev_vals, ',Sign Rank');
fprintf(median_vals, ',Sign Rank');
fprintf(skew_vals, ',Sign Rank');

for i=1:height(metric_labels)
   stat_test_result_print(mean_signed_rank_results(i,3), mean_vals);
   stat_test_result_print(stdev_signed_rank_results(i,3), stdev_vals);
   stat_test_result_print(median_signed_rank_results(i,3), median_vals);
   stat_test_result_print(skew_signed_rank_results(i,3), skew_vals);
end
...
fprintf(mean_vals, ',Z Stat');
fprintf(stdev_vals, ',Z Stat');
fprintf(median_vals, ',Z Stat');
fprintf(skew_vals, ',Z Stat');

for i=1:height(metric_labels)
   stat_test_result_print(mean_signed_rank_results(i,4), mean_vals);
   stat_test_result_print(stdev_signed_rank_results(i,4), stdev_vals);
   stat_test_result_print(median_signed_rank_results(i,4), median_vals);
   stat_test_result_print(skew_signed_rank_results(i,4), skew_vals);
end
...
for i = 1:height(metric_labels)
    [h,p,stats] = signrank(mean_positive_values(i,:),mean_control_values(i,:));
    mean_signed_rank_results(i,:) = [h,p,stats.signrank,stats.zval];
    [h,p,stats] = signrank(stdev_positive_values(i,:),stdev_control_values(i,:));
    stdev_signed_rank_results(i,:) = [h,p,stats.signrank,stats.zval];
    [h,p,stats] = signrank(median_positive_values(i,:),median_control_values(i,:));
    median_signed_rank_results(i,:) = [h,p,stats.signrank,stats.zval];
    [h,p,stats] = signrank(skew_positive_values(i,:),skew_control_values(i,:));
    skew_signed_rank_results(i,:) = [h,p,stats.signrank,stats.zval];
end
...
% print p vals
pos_con_label = 'Positive-Control Signed Rank Test';
...
for i = 1:height(metric_labels)
    [h,p,stats] = signrank(mean_negative_values(i,:),mean_control_values(i,:));
    mean_signed_rank_results(i,:) = [h,p,stats.signrank,stats.zval];
    [h,p,stats] = signrank(stdev_negative_values(i,:),stdev_control_values(i,:));
    stdev_signed_rank_results(i,:) = [h,p,stats.signrank,stats.zval];
    [h,p,stats] = signrank(median_negative_values(i,:),median_control_values(i,:));
    median_signed_rank_results(i,:) = [h,p,stats.signrank,stats.zval];
    [h,p,stats] = signrank(skew_negative_values(i,:),skew_control_values(i,:));
    skew_signed_rank_results(i,:) = [h,p,stats.signrank,stats.zval];
end
...
% print p vals
neg_con_label = 'Negative-Control Signed Rank Test';
